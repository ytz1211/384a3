    ...
    for i in range(len(initial_tenner_board[0])):
        for j in range(10):
            ...
            if i != len(initial_tenner_board[0])-1:  # bottom
                sat_tuples = []
                con = Constraint("BotC(Q{},Q{})".format(
                    i, j), [vars[i][j], vars[i+1][j]])
                ...
                if j != 0:  # bottom left
                    sat_tuples = []
                    con = Constraint("BLC(Q{},Q{})".format(
                        i, j), [vars[i][j], vars[i+1][j-1]])
    ...
    for i in range(len(initial_tenner_board[0])):
        ...
        for j in range(10):
            if initial_tenner_board[0][i][j] != -1:
                ...
                dom.remove(initial_tenner_board[0][i][j])
            else:
